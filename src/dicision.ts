...
const filterList = (
  ...
  [
    list.slice(0, sourceIndex).filter((v) => !!v),
    list.slice(sourceIndex + 1).filter((v) => !!v),
...
const getLineValues = (
  ...
  filterList(
    board.map((_, i) => {
      ...
      return col > -1 && col < 9 ? [i, col] : undefined;
    }),
    currentRow
...
const getFlipLineGetter =
  ...
  ([prevLine, nextLine]: [[number, number][], [number, number][]]) => {
    ...
    const beforeSameLastIndex = prevLine.findLastIndex(
      ([r, c]) => board[r][c] === value
    );
    ...
    const afterSameFirstIndex = nextLine.findIndex(
      ([r, c]) => board[r][c] === value
    );
